...
angular.module('myPostApp', [])
  .controller('myPostController', function ($scope, $http,$timeout) {
  ...
  $scope.getFeed = function(){$http({url:'/dashboard/client/myposts/fetch',method:'POST',headers: {'Content-Type': 'application/json'},data:{}})
   ...
   $scope.initFeed = function(){$http({url:'/dashboard/client/myposts/fetch',method:'POST',headers: {'Content-Type': 'application/json'},data:{}})
        .success(function(data, status, headers, config) {
        	//console.log(data);console.log(status);console.log(config);
        	//console.log(data.rurl);
        	//$window.location.href = data.rurl;
